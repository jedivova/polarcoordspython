...
def reproject_image_into_polar(data, origin=None):
    ...
    r_i, theta_i = Get_polar_axes(data.shape, 300, 451)
    ...
    coords = np.vstack((yi, xi)).T # (map_coordinates requires a 2xn array)
    print(coords.shape)
    print(data.shape)
    ...
    output = KNN_predict(data,coords)
...
def KNN_predict(IMG, coords):
    shape = IMG.shape
    X = [[i,j] for i in range(shape[0]) for j in range(shape[1])]
    Y = IMG.reshape(-1)
    knn = neighbors.KNeighborsRegressor(12, weights='distance')
    output = knn.fit(X, Y).predict(coords).reshape((300,451))
